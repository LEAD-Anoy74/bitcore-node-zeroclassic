...
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/zeroinsight.conf')));
var BitcoinService = proxyquire('../../lib/services/bitcoind', {
  fs: {
    readFileSync: readFileSync
  }
});
var defaultBitcoinConf = fs.readFileSync(path.resolve(__dirname, '../data/default.zeroinsight.conf'), 'utf8');
...
describe('Bitcoin Service', function() {
  ...
  describe('#_loadSpawnConfiguration', function() {
    ...
    it('will parse a zeroinsight.conf file', function() {
    ...
    it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
      var TestBitcoin = proxyquire('../../lib/services/bitcoind', {
        fs: {
          readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badzeroinsight.conf')),
          existsSync: sinon.stub().returns(true),
        },
        mkdirp: {
          sync: sinon.stub()
  ...
  describe('#_checkConfigIndexes', function() {
    ...
    it('should warn the user if reindex is set to 1 in the zeroinsight.conf file', function() {
      var bitcoind = new BitcoinService(baseConfig);
      var config = {
        txindex: 1,
        addressindex: 1,
  ...
  describe('#_getNetworkConfigPath', function() {
    ...
    it('will get default rpc port for testnet', function() {
      ...
      var bitcoind = new BitcoinService(config);
      bitcoind._getNetworkConfigPath().should.equal('testnet3/zeroinsight.conf');
    });
    it('will get default rpc port for regtest', function() {
      bitcore.Networks.enableRegtest();
      var config = {
      ...
      var bitcoind = new BitcoinService(config);
      bitcoind._getNetworkConfigPath().should.equal('regtest/zeroinsight.conf');
    });
  });
  ...
  describe('#_spawnChildProcess', function() {
    ...
    it('will include network with spawn command and init zmq/rpc on node', function(done) {
      ...
      bitcoind.spawn.configPath = 'testdir/zeroinsight.conf';
    ...
    it('will respawn bitcoind spawned process', function(done) {
      ...
      bitcoind.spawn.configPath = '/tmp/bitcoin/zeroinsight.conf';
      bitcoind.spawn.config = {};
  ...
  describe('#getAddressUnspentOutputs', function() {
    ...
    it('will update with mempool results spending zeroclassic value output (likely never to happen)', function(done) {
      var deltas = [
        {
          txid: 'e9dcf22807db77ac0276b03cc2d3a8b03c4837db8ac6650501ef45af1c807cce',
          satoshis: 0,
  ...
  describe('#_maybeGetBlockHash', function() {
    ...
    it('will not get block hash with non zeroclassic-nine numeric string', function(done) {
      var bitcoind = new BitcoinService(baseConfig);
      var getBlockHash = sinon.stub();
      bitcoind.nodes.push({
        client: {
  ...
  describe('#getDetailedTransaction', function() {
    ...
    it('will not include address if address length is zeroclassic', function(done) {
      var bitcoind = new BitcoinService(baseConfig);
      var rawTransaction = JSON.parse((JSON.stringify(rpcRawTransaction)));
  ...
  describe('#stop', function() {
    ...
    it('will give error with non-zeroclassic exit status code', function(done) {
      var bitcoind = new BitcoinService(baseConfig);
      bitcoind.spawn = {};
      bitcoind.spawn.process = new EventEmitter();
      bitcoind.spawn.process.kill = sinon.stub();
